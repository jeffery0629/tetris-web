...
class OnlineBattleGame(BattleGame):
    ...
    async def connect_and_wait(self, player_name: str = "Player"):
        ...
        # Wait for MATCH_START or WAITING
    ...
    async def run_async(self, player_name: str = "Player"):
        ...
        if not connected:
            ...
            while True:
                ...
                hint = self.font_small.render("Press ESC to return", True, COLOR_TEXT)
    ...
    def _handle_opponent_state(self, data):
        ...
        # Note: We store piece data but don't control it
        if 'piece' in data:
            piece_data = data['piece']
            # Store for rendering (could create a simple visual-only block)
            remote_player._remote_piece = piece_data
    ...
    def _handle_input_online(self):
        ...
        # Player 1: WASD
        if self.local_role == 1:
            if keys[pygame.K_a] and current_time - inputs.get('left', 0) > self.input_delay:
                self._move_block(local_player, -1, 0)
                inputs['left'] = current_time
            if keys[pygame.K_d] and current_time - inputs.get('right', 0) > self.input_delay:
                self._move_block(local_player, 1, 0)
                inputs['right'] = current_time
            if keys[pygame.K_s] and current_time - inputs.get('down', 0) > self.input_delay:
    ...
    def _update_online(self, dt: float):
        ...
        if local_player.pending_garbage > 0:
            self._apply_garbage(local_player)
